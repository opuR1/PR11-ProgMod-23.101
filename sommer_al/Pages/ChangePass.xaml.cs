...
namespace sommer_al.Pages
{
    public partial class ChangePass : Page
    {
        ...
        private async void btnSendCode_Click(object sender, RoutedEventArgs e)
        {
            ...
            bool isEmployee = rbEmployee.IsChecked ?? false;

            if (string.IsNullOrEmpty(loginOrEmail))
            {
                ShowStep1Error("Введите email");
                return;
            }
            ...
            try
            {
                ...
                if (_recoveryService.StartPasswordRecovery(loginOrEmail, isEmployee, out _userEmail, out string recoveryCode))
                {
                ...
                {
                    ShowStep1Error("Пользователь с такой ролью и email не найден");
                }
            }
            catch (Exception ex)
            {
                ShowStep1Error("Ошибка при отправке кода");
            }
            finally { btnSendCode.IsEnabled = true; }
        }
        ...
        private async void btnResendCode_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                string newCode = _recoveryService.UpdateRecoveryCode(_userEmail);
        ...
        {
            StopAllTimers();
            MessageBox.Show($"Пытаюсь сменить пароль для: {_userEmail}");
